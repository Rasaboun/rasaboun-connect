...
import { ItineraireItem } from './ItineraireItem'

// Local, minimal motion: results fade up in a quick stagger on mount. Shares the
// same easing curve as the screen transitions in App for a consistent feel.
...
export function ItineraireList({
  ...
  return (
    <m.div
      className="flex flex-col space-y-4 px-4 lg:w-2/4 lg:px-0 lg:pl-28"
      initial="initial"
      animate="animate"
      variants={container}
    >
      <div className="mt-4 flex flex-row items-center space-x-0 md:space-x-4">
        <button
          className="hidden flex-row items-center space-x-1.5 rounded-xl bg-white fill-[#127996] px-6 py-2 text-[#127996] transition hover:bg-[#E5E5EE] disabled:cursor-not-allowed disabled:opacity-60 lg:inline-flex"
          disabled={refreshing}
          onClick={onRefresh}
          type="button"
        >
          <p>{refreshing ? 'Actualisation…' : 'Actualiser'}</p>
          <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${refreshing ? 'animate-spin' : ''}`}>
      ...
      <m.div
        className="overflow-hidden rounded-2xl bg-[#3057D3] p-1.5 shadow-[0_4px_14px_rgba(48,87,211,0.25)]"
        variants={item}
      >
        <div className="flex items-center gap-2 px-2.5 py-2 text-white">
          <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="h-5 w-5">
            <path d="M9 18h6M10 21h4M12 3a6 6 0 0 0-3.5 10.9c.6.45 1 1.13 1 1.9V16h5v-.2c0-.77.4-1.45 1-1.9A6 6 0 0 0 12 3z" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          <span className="text-[15px] font-black">Trajet recommandé</span>
        </div>
        <ItineraireItem
          journey={journeys[0]}
          selected={journeys[0].id === selectedId}
          onSelect={() => onSelect(journeys[0].id)}
          recommended
        />
      </m.div>
      ...
      {journeys.slice(1).map((journey) => (
        <m.div key={journey.id} variants={item}>
          <ItineraireItem
            journey={journey}
            selected={journey.id === selectedId}
            onSelect={() => onSelect(journey.id)}
          />
        </m.div>
      ))}
      ...
      <div className="flex w-full flex-row items-center justify-center space-x-2 rounded-xl bg-white p-2 py-3 text-[#127996] hover:bg-[#E5E5EE]">
        <p>Voir + de trajets</p>
        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="h-4 w-4 fill-[#127996]">
