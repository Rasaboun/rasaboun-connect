import type { JourneySection } from '../../travel/navitia'
import { ModeIcon } from '../ModeIcon'

const TRAIN_MODES = ['Train', 'Transilien', 'RailShuttle', 'LocalTrain', 'TER', 'TER / Intercités']

export function TransportLogo({ elem }: { elem: JourneySection }) {
  if (elem.mode === 'Métro')
    return (
      <div className="flex flex-row items-center space-x-0.5">
        ...
        <span
          className="h-5 w-5 rounded-full text-center text-sm font-semibold text-white"
  ...
  if (elem.mode === 'RER')
    return (
      <div className="flex flex-row items-center space-x-0.5">
        ...
        <span
          className="h-5 w-5 rounded-md text-center text-sm font-semibold text-white"
          style={{ backgroundColor: elem.color ?? '#999' }}
        >
          {elem.label}
        </span>
  ...
  if (elem.mode === 'Bus')
    return (
      <div className="flex flex-row items-center space-x-0.5">
        ...
        <span
          className="h-5 w-fit rounded px-0.5 text-center text-sm font-semibold text-white"
          style={{ backgroundColor: elem.color ?? '#999' }}
        >
          {elem.label}
        </span>
  ...
  if (elem.mode === 'Tramway')
    return (
      <div className="flex flex-row items-center space-x-0.5">
        ...
        <span
          className="h-fit w-6 border-b-[3px] border-t-[3px] text-center text-xs font-semibold text-black"
          style={{ borderColor: elem.color ?? '#999' }}
        >
          {elem.label}
        </span>
      </div>
    )
  if (TRAIN_MODES.includes(elem.mode))
    return (
      <div className="flex flex-row items-center space-x-0.5">
  ...
  return (
    <span
      className="inline-flex h-5 items-center rounded-md px-2 text-xs font-semibold text-white"
      style={{ backgroundColor: elem.color ?? '#999' }}
    >
      {elem.label}
    </span>
